...
import { testConnection, initializeDatabase, getDatabase, rowToExpense, User, Tag, Expense } from './src/lib/database.js';
...
app.post('/api/documents', upload.single('file'), async (req, res) => {
  try {
    ...
    const newStmt = db.prepare('SELECT * FROM expenses WHERE id = ?');
    const newDoc = newStmt.get(document_id);
...
app.put('/api/documents/:id', upload.single('file'), async (req, res) => {
  try {
    ...
    const newStmt = db.prepare('SELECT * FROM expenses WHERE id = ?');
    const updatedDoc = newStmt.get(id);
...
app.delete('/api/documents/:id', async (req, res) => {
  try {
    ...
    const db = getDatabase();
    const stmt = db.prepare('SELECT receipt_url FROM expenses WHERE id = ?');
    const doc = stmt.get(id);
    
    if (doc && doc.receipt_url) {
      const filePath = path.join(__dirname, doc.receipt_url);
